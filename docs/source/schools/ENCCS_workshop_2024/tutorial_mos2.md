...
After enabling [myst-parser](https://myst-parser.readthedocs.io/en/latest/using/syntax-optional.html), in .md files math mode can be accessed with dollar signs

$$ 
\int e^x = e^x i
$$ (eqn:intexp)

Using labels, these equations can be referenced to: {eq}`eqn:intexp`.
...
````{seealso} [OPTIONAL]: Use the terminator to accelerate convergence
...
```{figure} ref-Guandalini.png
:scale: 40%
```
_Guandalini, D’Amico, Ferretti & Varsano. npj Comput Mater 9_

````{seealso} [OPTIONAL]: Use the RIM-W accelerator
...
````{seealso} [OPTIONAL] Comparison with CPU calculation with hybrid parallelization strategy
...
.
````{seealso} [OPTIONAL]: Comparing different parallelisation schemes
